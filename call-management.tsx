...
//     // First, we need to get the war data to know which faction is the enemy
...
//       const calledTargets = Object.values(enemyData.faction?.members || {}).filter((member)=>requestedTargetIds.has(String(member.id))).map((member)=>({
//           id: member.id,
//           user_id: member.id,
//           name: member.name,
//           level: member.level,
...
//     const availableTargets = Object.values(enemyData.faction?.members || {}).filter((member)=>!calledTargetIds.has(member.id)).map((member)=>({
//         user_id: member.id,
//         name: member.name,
//         level: member.level,
//         faction_id: enemyFactionId,
